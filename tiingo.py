...
while start_date < end_date:
    url = f"https://api.tiingo.com/tiingo/fx/{ticker}/prices?resampleFreq=1min&token={API_TOKEN}&startDate={start_date.strftime('%Y-%m-%d')}&endDate={(start_date + delta).strftime('%Y-%m-%d')}"
    response = requests.get(url, headers=headers)
    data = response.json()
    if data:
...
if all_data:
    ...
    with open(f'downloads/{ticker}.csv', 'w', newline='') as output_file:
